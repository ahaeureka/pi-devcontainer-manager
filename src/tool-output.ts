...
export function formatToolSize(bytes: number): string {
...
export function formatToolOutput(
  output: string,
  opts: { maxBytes?: number; maxLines?: number; prefix?: string } = {},
): ToolOutputResult {
  ...
  for (let i = allLines.length - 1; i >= 0; i--) {
    ...
    if (kept.length + 1 > maxLines || keptBytes + lineBytes > maxBytes) break;
    kept.unshift(line);
    keptBytes += lineBytes;
  }
  if (kept.length === 0) {
    ...
    return {
      text: `${prefix.length > 0 ? `${prefix}\n` : ""}${slice}\n[Output truncated: showing first ${formatToolSize(
        maxBytes,
      )} of a single ${formatToolSize(totalBytes)} line. Full output: ${fullOutputPath}]`,
      truncated: true,
      fullOutputPath,
    };
  }
  ...
  const notice = `\n[Showing lines ${startLine}-${endLine} of ${totalLines} (${formatToolSize(
    totalBytes,
  )} total, kept ${formatToolSize(keptBytes)}). Full output: ${fullOutputPath}]`;
  return {
    text: `${prefix.length > 0 ? `${prefix}\n` : ""}${keptText}${notice}`,
    truncated: true,
    fullOutputPath,
...
export function persistFullOutput(output: string): string {
