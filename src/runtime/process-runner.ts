 ...
 */
export function toSpawnError(file: string, error: unknown): RuntimeError {
  ...
  if (code === "ENOTCONN") {
    return new RuntimeError({
      ...
      cause: error,
      remedy:
        "A filesystem mount that PATH points into is disconnected (a stale AppImage /tmp/.mount_* entry does this), which fails the executable search before it reaches the binary. Unmount it (`fusermount3 -u <dir>`) or drop it from PATH, then retry: `env node --version` reproduces the failure.",
    });
  }
  return new RuntimeError({
    kind: "unexpected",
