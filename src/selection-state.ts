...
export type SelectionIntent =
  | { readonly kind: "selected"; readonly record: SelectionRecordV2 | SelectionRecordV1 }
...
export function encodeSelectionRecord(record: SelectionRecord | SelectionRecordV1): string {
  return JSON.stringify(record);
}

export function parseSelectionRecord(payload: string): SelectionRecordV2 | undefined {
  const intent = parseSelectionIntent(payload);
  if (intent === undefined || intent.kind !== "selected") return undefined;
...
export function parseSelectionIntent(payload: string): SelectionIntent | undefined {
  let parsed: unknown;
  try {
...
export function normalizeSelectionIntent(value: unknown): SelectionIntent | undefined {
  ...
  if (record.state !== "selected") return undefined;
  const normalized = normalizeSelectionRecord({ ...record, version: 1 });
  const workspaceKey = normalized?.workspaceKey;
  if (normalized === undefined || workspaceKey === undefined) return undefined;
...
export function normalizeSelectionRecord(value: unknown): SelectionRecordV2 | undefined {
  if (typeof value !== "object" || value === null) return undefined;
  const record = value as Record<string, unknown>;
  if (record.version !== 1 && record.version !== SELECTION_PAYLOAD_VERSION) return undefined;
