 ...
 */
function skipComment(text: string, from: number): number | undefined {
  ...
  if (next === "*") {
    ...
    if (i + 2 > text.length) throw new SyntaxError("Unterminated block comment");
    return i + 2;
  }
  return undefined;
}
  ...
  while (i < input.length) {
    ...
    if (ch === ",") {
      ...
      for (;;) {
        ...
        if (pastComment === undefined) break;
