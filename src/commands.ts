...
export async function reconcileSelection(
  ...
  const usableId =
    entry.ambiguous ? undefined : offered ? persisted : primaryCandidate(entry)?.id;
...
export function createCommandHandlers(services: CommandServices): Record<string, (args: string, ctx: CommandContextLike) => Promise<CommandResult>> {
  ...
  const chooseAmbiguousContainer = async (
    ...
    const label = (c: RegistryEntry["containerCandidates"][number], short: boolean): string =>
      `${short ? c.id.slice(0, 12) : c.id} — ${c.state}${c.image !== undefined ? ` — ${c.image}` : ""}`;
  ...
  const withTarget = (text: string): CommandResult =>
    ...
    if (candidates.length === 1) {
      ...
      if (failure !== undefined) return { text: failure };
      return withTarget(
        `Selected \`${only.workspacePath}\` (${containerStateOf(only) ?? "config-only"}).\nRun /devcontainer up if it is not running.`,
      );
    }
...
function findNextArgvWord(text: string): { index: number; match: string } {
  const re = /(?:^|\s)(--argv(?==|\s|$))/g;
