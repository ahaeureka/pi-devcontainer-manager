 ...
 */
import { shellForm } from "./execution-service.js";
 ...
 */
export function createRoutedBashOperations(options) {
    return {
        exec: (command, cwd, execOptions) => new RoutedBashRouter(options).exec(command, cwd, execOptions),
    };
}
/**
...
class RoutedBashRouter {
    ...
    async exec(command, cwd, execOptions) {
        ...
        if (outcome.stderr.length > 0)
            execOptions.onData(Buffer.from(outcome.stderr, "utf8"));
     ...
     */
    resolveWorkspace(reportedCwd) {
        if (!isAbsolute(reportedCwd))
            return this.options.sessionWorkspace;
    ...
    sanitizeEnvironment(env) {
        ...
        for (const [name, value] of Object.entries(env)) {
            if (typeof value === "string" && allowlist.includes(name))
                result[name] = value;
        }
        return Object.keys(result).length > 0 ? result : undefined;
    }
}
//# sourceMappingURL=bash-router.js.map