...
export function createDevcontainerExecTool(options) {
    return {
        ...
        description: "Execute an argv command inside the currently selected DevContainer target. " +
...
export function createDevcontainerStatusTool(status) {
    return {
        name: DEV_CONTAINER_STATUS_TOOL,
        label: "Dev Container Status",
        description: "Read-only summary of the current DevContainer selection and workspace registry. " +
 ...
 */
export function createDevcontainerHostExecTool(options) {
    return {
        ...
        execute: async (_toolCallId, params, signal, _onUpdate, _ctx) => {
            if (!options.hostExecutionAllowed) {
                // The RAW first token: the ledger renders it (rendering twice turned "(no command)" into "(no").
                options.onWithheldHostAttempt?.(params.argv[0] ?? "");
