...
export async function applySelection(services, target, ctx) {
...
export function establishedTarget(selection) {
    if (selection.workspaceKey === undefined)
        return undefined;
    if (selection.status !== "selected-valid" && selection.status !== "selected-stopped")
        return undefined;
 ...
 */
export async function reconcileSelection(services, ctx, hint, 
/**
...
preloaded) {
    ...
    const carriedConfig = hint.configPath ??
...
function selectorNameOf(candidate) {
    if (candidate.configKind !== ".devcontainer/<name>/devcontainer.json")
        return "default";
 ...
 */
export function configPathFor(entry, requested) {
    if (requested.selector !== undefined)
        return resolveConfigCandidate(entry, requested.selector);
...
export function resolveConfigCandidate(entry, requested) {
    const discovered = candidatesOf(entry);
...
export function createCommandHandlers(services) {
    const handlers = {};
    ...
    handlers["use"] = async (args, ctx) => {
        ...
        let established;
        const select = async (entry, candidateId) => {
            const resolved = configSelector === undefined ? { ok: true } : resolveConfigCandidate(entry, configSelector);
     ...
     * one (AC-2).
     */
    const resolveUpBuildTarget = async (args, ctx) => {
    ...
    handlers["setup"] = async (_args, ctx) => {
        ...
        const confirmed = await ctx.ui.confirm("Install Dev Containers CLI", "This runs \"npm install -g @devcontainers/cli\" (installs or upgrades the CLI globally on the HOST). Continue?", ctx.signal !== undefined ? { signal: ctx.signal } : undefined);
        if (!confirmed)
...
async function lifecycleCommand(services, ctx, action, _args) {
    const snapshot = services.targetStore.snapshot();
    const container = resolveContainer(snapshot);
    if (container === undefined) {
    ...
    const confirmed = await ctx.ui.confirm(`Confirm ${action}`, `${action === "stop" ? "Stop" : "Remove"} container \`${container.name}\` (${container.id.slice(0, 12)})?`, ctx.signal !== undefined ? { signal: ctx.signal } : undefined);
    if (!confirmed)
...
function resolveContainer(snapshot) {
    if (snapshot.candidateId === undefined)
        return undefined;
    return {
